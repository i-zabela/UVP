...
def zdruzi_sladkosnede(domaci, njihovi):
    skupni = domaci
    for clan in njihovi:
        if clan not in domaci.keys():
            skupni[clan] = njihovi[clan]
    return skupni
# =====================================================================@020339=
# 3. podnaloga
...
def nakupovalni_seznam(sladkosnedi, gostje):
    sladice = {None: 0}
    for oseba in gostje:
        cukr = sladkosnedi.get(oseba)
        sladice[cukr] = sladice.get(cukr, 0) + 1
    sladice.pop(None)
    return sladice
...
def nakupovalni_seznam(sladkosnedi, gostje):
    pojavitve = {}
    for ime in gostje:
        if ime in sladkosnedi:
            sladica = sladkosnedi[ime]
            pojavitve[sladica] = pojavitve.get(sladica, 0) + 1
    return pojavitve
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzNywidXNlciI6MTE1MTR9:1x15iH:1yOMHM21LsORXjLUZWcphlfp3n0ydkMnv4RW389lfl8"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzOCwidXNlciI6MTE1MTR9:1x15iH:PTvRO4yclONvMI2bUfB5CrY3f43DlR6FwTlmv098lRg"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoyMDMzOSwidXNlciI6MTE1MTR9:1x15iH:LaB0KFhqSBmyBpe8LcFjki9duSWrgtLV8UKk1kXVlV4"
        try:
