...
def najpogostejse_besede(vhod, st_besed, izhod):
    ...
    for beseda in besede:
        sestej[beseda] = sestej.get(beseda, 0) + 1

    urejeno = sorted(sestej.items(), key=lambda par: (-par[1], par[0]))

    with open(izhod, 'w', encoding='utf-8') as dat:
        for beseda, stevilo in urejeno[:st_besed]:
            dat.write(f'{beseda} {stevilo}\n')
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODkxLCJ1c2VyIjoxMTUxNH0:1x2TfE:Kn0TK9N9IAmBdhGo0dGt3hf8f6ahrfyd_4KzH_eItxQ"
        try:
            pass
        except TimeoutError:
